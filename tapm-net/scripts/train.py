...
def evaluate_and_predict_eol(cfg, model, test_dataset, eol_threshold=80.0):
    ...
    for cell_id, info in cells_data.items():
        ...
        plt.plot(known_cycles, true_soh, label='实际测量 SOH', color='black', linestyle='--')
        plt.plot(projected_cycles, projected_soh, label='TAPM-Net 连续外推趋势', color='red', linewidth=2)
